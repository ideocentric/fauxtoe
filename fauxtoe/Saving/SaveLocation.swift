...
/// The folder photos are saved to. Defaults to ~/Pictures/fauxtoe; a folder chosen by the user is kept
...
final class SaveLocation {
    ...
    /// The real ~/Pictures/fauxtoe. FileManager's home directory points inside the sandbox container.
    static var defaultFolder: URL {
        let home = getpwuid(getuid()).flatMap { String(validatingCString: $0.pointee.pw_dir) } ?? NSHomeDirectory()
        return URL(fileURLWithPath: home, isDirectory: true)
            .appendingPathComponent("Pictures", isDirectory: true)
            .appendingPathComponent("fauxtoe", isDirectory: true)
    }
